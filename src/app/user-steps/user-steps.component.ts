...
export class UserStepsComponent implements OnInit {
  ...
  private calculateData() {
    ...
    if (this.steps !== undefined) {
      let stepsoftheday = this.steps[this.day.format('YYYY-MM-DD')];

      if (stepsoftheday !== undefined) {
        let numberofsteps = 0;


        //let sortedSteps = _.sortBy(stepsoftheday, ['startDate']);
        //let sortedSteps = stepsoftheday;

        let sortedSteps = [];
        for (let step in stepsoftheday) {
          sortedSteps.push(stepsoftheday[step]);
        }
        sortedSteps.sort(this.compare);

        for (let step in sortedSteps) {
          let datetimeStart = moment.utc(sortedSteps[step]['startDate']).startOf('minute').valueOf();
          let datetimeEnd = moment.utc(sortedSteps[step]['endDate']).startOf('minute').valueOf();
          numberofsteps += sortedSteps[step]['value'];

          if (this.chart !== undefined) {
            console.log(datetimeStart + ':' + numberofsteps);
            console.log(datetimeEnd + ':' + numberofsteps);

            let bundleId = sortedSteps[step].sourceBundleId;
            ...
            sources[bundleId].push([datetimeStart, numberofsteps]);
            sources[bundleId].push([datetimeEnd, numberofsteps]);
            sources[bundleId].push([datetimeEnd, null]);

            //this.chart.series[0].addPoint([datetimeStart, numberofsteps]);
            //this.chart.series[0].addPoint([datetimeEnd, numberofsteps]);
            //this.chart.series[0].addPoint([datetimeEnd, null]);
  ...
  round(date) {
    ...
    let roundedMintues = (10 * Math.ceil(minutes / 10));


    return moment(date).startOf('hour').add(roundedMintues, 'minute');
  }
